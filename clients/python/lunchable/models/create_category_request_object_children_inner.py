...
class CreateCategoryRequestObjectChildrenInner(BaseModel):
    ...
    oneof_schema_3_validator: Optional[ChildCategoryObject] = None
    actual_instance: Optional[Union[ChildCategoryObject, int, str]] = None
    one_of_schemas: Set[str] = {"ChildCategoryObject", "int", "str"}
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in CreateCategoryRequestObjectChildrenInner with oneOf schemas: ChildCategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in CreateCategoryRequestObjectChildrenInner with oneOf schemas: ChildCategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into CreateCategoryRequestObjectChildrenInner with oneOf schemas: ChildCategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into CreateCategoryRequestObjectChildrenInner with oneOf schemas: ChildCategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ChildCategoryObject, int, str]]:
