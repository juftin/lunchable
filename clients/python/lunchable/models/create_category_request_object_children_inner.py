...
class CreateCategoryRequestObjectChildrenInner(BaseModel):
    ...
    oneof_schema_1_validator: Optional[StrictInt] = Field(
        default=None, description="ID of an existing category."
    )
    # data type: str
    oneof_schema_2_validator: Optional[
        Annotated[str, Field(strict=True, max_length=100)]
    ] = Field(default=None, description="Name of a new child category to be created.")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, CategoryObject):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `CategoryObject`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in CreateCategoryRequestObjectChildrenInner with oneOf schemas: CategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in CreateCategoryRequestObjectChildrenInner with oneOf schemas: CategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into CreateCategoryRequestObjectChildrenInner with oneOf schemas: CategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into CreateCategoryRequestObjectChildrenInner with oneOf schemas: CategoryObject, int, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
