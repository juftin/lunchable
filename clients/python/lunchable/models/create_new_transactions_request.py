...
class CreateNewTransactionsRequest(BaseModel):
    ...
    transactions: Annotated[
        List[InsertTransactionObject], Field(min_length=1, max_length=500)
    ] = Field(description="List of transactions to insert.")
    ...
    skip_duplicates: Optional[StrictBool] = Field(
        default=False,
        description="If set to true, the system will flag new transactions that have the same `date`, `payee`, `amount`, and account_id (plaid or manual), as an existing transaction, as a duplicate. <br><br> Note that deduplication based on `external_id` will always occur regardless of how this property is set.",
    )
