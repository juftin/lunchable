...
class RecurringObject(BaseModel):
    ...
    def source_validate_enum(cls, value):
        ...
        if value not in set(["manual", "transaction", "system"]):
            raise ValueError(
                "must be one of enum values ('manual', 'transaction', 'system')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "transaction_criteria": RecurringObjectTransactionCriteria.from_dict(
                    obj["transaction_criteria"]
                )
                if obj.get("transaction_criteria") is not None
                else None,
                "overrides": RecurringObjectOverrides.from_dict(obj["overrides"])
                if obj.get("overrides") is not None
                else None,
                "matches": RecurringObjectMatches.from_dict(obj["matches"])
